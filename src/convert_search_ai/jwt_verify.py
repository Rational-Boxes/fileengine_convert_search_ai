...
the shared ``FILEENGINE_JWT_SECRET``, so the service authorizes straight from the
signed claims without an introspection round-trip.

Security: the algorithm is pinned to HS256 (``alg: none`` / RS-confusion tokens
are rejected), the signature is compared in constant time, and ``exp`` is enforced.
"""
